...
import 'package:public_transit_tracking_app/Assistants/mapKitAssistant.dart';
...
class _NewRideScreenState extends State<NewRideScreen>
{
  ...
  void acceptRideRequest()
  {
    ...
    newRequestsRef.child(rideRequestId).child("VehicleNumber").set(driversInformation.VehicleNumber);

    double lat; double lng;
    availableDriversRef.child(firebaseUserCurrent.value.uid).once().then((DataSnapshot dataSnapShot){
      lat = double.parse(dataSnapShot.value['l']['0'].toString());
      lng = double.parse(dataSnapShot.value['l']['1'].toString());
    });
    Map locMap =
    {
      //"latitude": currentPosition.latitude.toString(),
    ...
    taxiDriversRef.child(currentfirebaseUser.uid).child("history").child(rideRequestId).set(true);
  }
